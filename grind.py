...
def upgradable(up_cost, level, color="none"):
    ...
    if color == "none":
        if money >= up_cost:
            return "\033[32ml: " + str(level) + "\033[0m" + " " * (5 - len(level)) + "|"
        else:
            return "\033[31ml: " + str(level) + "\033[0m" + " " * (5 - len(level)) + "|"
    else:
        if res_all[color]["count"] >= up_cost:
            return "\033[32ml: " + str(level) + "\033[0m" + " " * (5 - len(level)) + "|"
        else:
            return "\033[31ml: " + str(level) + "\033[0m" + " " * (5 - len(level)) + "|"
    ...
    if i:
        if index[i["name"]] < 10:
            print(
                ...
                pn(i["count"] * i["price"]) + "$",
                "\nДо заполнения :",
                text_time((i["storage"] - i["count"]) / i["per_s"]),
                "\nУровень       :",
                pn(i["level"]),
                "\n\nУлучшение\n",
            )
            ...
            if (i["level"] + 1) % 50 == 0:
                print("Хранилище   :", "+" + pn(i["storage"], recolor=1))
            else:
                print("Хранилище   :", "+" + pn(i["level"] * 1.5, recolor=1))

            if (i["level"] + 1) % 100 == 0:
                print(
                    "Стоимость 1 :",
                    "+" + pn(i["price"] + i["price_start"] * 2, recolor=1) + "$",
                )
            else:
                print("Стоимость 1 :", "+" + pn(i["price_start"], recolor=1) + "$")

            print(
                "В секунду   :",
                "+0.1",
                "\n\nЦена улучшения :",
                pn(i["up_cost"], recolor=1) + "$",
                "\nДеньги         :",
                pn(money, recolor=1) + "$",
                "\033[0m",
            )
            ...
            if res_all[index[i["name"]] - 10]["count"] >= i["up_cost"]:
                print("\033[32m", end="")
                color = "\033[32m"
            else:
                print("\033[31m", end="")
            ...
            print(
                "\nЦена улучшения :",
                pn(i["up_cost"], recolor=1) + " " + text[index[i["name"]] - 10],
            )
            print(
                "{:15}".format(res_all[index[i["name"]] - 10]["name"]) + ":",
                pn(res_all[index[i["name"]] - 10]["count"], recolor=1),
                "\033[0m",
            )
...
def save_stats():
    ...
    if res_all:
        ...
        for i in res_all:
            if i["storage"] / i["per_s"] <= stats[8]:
                stats[8] = i["storage"] / i["per_s"]
            if i["level"] >= stats[13]:
                stats[13] = i["level"]
            stats[12] += i["level"]
...
def draw_stats():
    ...
    print(
        ...
        "\nЧасов получено  :",
        pn(stats[5]),
        "\n\nМаксимальное время в афк :",
        max_offline,
        "\nПрибыльное время в афк   :",
        best_offline,
        "\n\nДенег получено               :",
        pn(stats[6]) + "$",
        "\nМаксимум денег в кошельке    :",
        pn(stats[9]) + "$",
        "\nДеньги                       :",
        pn(money) + "$",
        "\n\nДенег в секунду за все время :",
        pn(stats[10]) + "$ / c",
        "\nДенег в секунду за ресурсы   :",
        pn(stats[14]) + "$ / c",
        end="",
    )
    if colors_open:
    ...
    print(
        "\nСуммарно денег в секунду     :",
        pn(stats[14] + stats[11]) + "$ / c",
        "\n\nУровней куплено      :",
        pn(stats[12]),
...
if os.path.exists("./save.dat"):
    with open("save.dat", "r") as f:
        ...
        if timestamp // 60 >= 1000:
            print(draw_name("Минуты"), pn(timestamp // 60))
        else:
            print(draw_name("Минуты"), timestamp // 60)

        if timestamp // 3600 >= 1000:
            print(draw_name("Часы"), pn(timestamp // 3600))
        else:
            print(draw_name("Часы"), timestamp // 3600)

        print(draw_name("Уровни"), pn(timestamp // 3600), end="\n\n")

        for i in res_all:
            if i["per_s"] * timestamp + i["count"] < i["storage"]:
                print(draw_name(i["name"]), pn(i["per_s"] * timestamp))
            else:
                print(
                    "\033[31m" + draw_name(i["name"]),
                    pn(i["storage"] - i["count"]) + "\033[0m",
                )
...
def draw_header():
    ...
    print(
        "Секунды   :",
        "{0:{1}}".format(pn(res_time["seconds"]), tab),
        "| Профиль :",
        name,
    )
    print(
        "Минуты    :",
        "{0:{1}}".format(pn(res_time["minutes"]), tab),
        "| Уровень :",
        pn(level),
    )
    print(
        "Часы      :",
        "{0:{1}}".format(pn(res_time["hours"]), tab),
        "| Деньги  :",
        pn(money) + "$",
    )
...
def draw_buy():
    if buy[0] <= 12:
        ...
        if buy[0] == 6:
            if (
                res_time["hours"] >= 15
                and res_all[3]["count"] >= 50000
                and money >= 10000000000
            ):
                buy[1] = 1
        if buy[0] == 7:
        ...
        if buy[0] == 11:
            if (
                ...
                and res_all[6]["count"] >= 150000
                and res_all[7]["count"] >= 150000
                and res_all[8]["count"] >= 150000
            ):
                buy[1] = 1
...
def draw_res():
    if res_all:
        ...
        for i in res_all:
            print(
                draw_name(i["name"]),
                draw_storage(i["count"], i["storage"]),
                "|",
                upgradable(i["up_cost"], i["level"]),
                pn(i["price"] * i["count"]) + "$",
            )
    ...
    if colors_open:
        ...
        for i in range(len(colors)):
            print(
                text[i],
                upgradable(colors[i]["up_cost"], colors[i]["level"], i),
                pn(colors[i]["per_s"] * colors[i]["level"]) + "$ / с",
            )
...
def game_render():
    while 1:
        ...
        if dev_mode:
            print(
                "\nws:",
                res_time["wait_seconds"],
                "\nwm:",
                res_time["wait_minutes"],
                "\nwh:",
                res_time["wait_hours"],
                "\nut:",
                time.time(),
        ...
        if move[0] == "open" and buy[0] <= 12:
            if buy[1] == 1:
                ...
                print("\033[31m" + buy[buy[0] + pn_mode * 11] + "\033[0m")
                move = input().split(" ")

        if (move[0] == "sell" and len(move) > 1) or move[0] == "sa":
            if move[0] == "sa":
                move = ["sell", "all"]
            elif move[1] != "all" and move[1].isalpha():
                move[1] = move[1].capitalize()
            sell(move[1])
            ...
            if i:
                while 1:
                    if not colored and money >= i["up_cost"]:
                        ...
                        colored
                        and colors_open
                        and res_all[index[i["name"]] - 10]["count"] >= i["up_cost"]
                    ):
